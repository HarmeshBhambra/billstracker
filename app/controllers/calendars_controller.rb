class CalendarsController < ApplicationController
  ...
  def index
    ...
    @week_1_bills_mon = @week_1_bills.select{|bill| bill.due_at.mon.to_s === @mon.to_s}

    @week_2_bills = current_user.bills.select{|bill| bill.due_at.day.between?(8, 14)}

    @week_2_bills_mon = @week_2_bills.select{|bill| bill.due_at.mon.to_s === @mon.to_s}

    @week_3_bills = current_user.bills.select{|bill| bill.due_at.day.between?(15, 21)}

    @week_3_bills_mon = @week_3_bills.select{|bill| bill.due_at.mon.to_s === @mon.to_s}

    @week_4_bills = current_user.bills.select{|bill| bill.due_at.day.between?(22, 28)}

    @week_4_bills_mon = @week_4_bills.select{|bill| bill.due_at.mon.to_s === @mon.to_s}

    @week_5_bills = current_user.bills.select{|bill| bill.due_at.day.between?(29, 31)}

    @week_5_bills_mon = @week_5_bills.select{|bill| bill.due_at.mon.to_s === @mon.to_s}
